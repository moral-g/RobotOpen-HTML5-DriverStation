...
_.extend(Backbone.LocalStorage.prototype, {
  ...
  hrspDup: function(resp,opt,synDf){
    if (resp) {
      if (opt && opt.success)
        opt.success(resp);
      if (synDf)
  ...
  create: function(model, options, syncDfd, handleResp) {
    ...
    this.save();
    var that = this;
    that.hrspDup(this.find(model), options, syncDfd);
  },
  ...
  update: function(model, options, syncDfd, handleResp) {
    ...
    if (!_.include(this.records, model.id.toString()))
      ...
      var that = this;
      that.hrspDup(model, options, syncDfd);
  },

  // Retrieve a model from `this.data` by id.
  find: function(model, options, syncDfd, handleResp) {
    var that = this;
    chrome.storage.local.get(this.name+"-"+model.id, function(data,options,syncDfd) {
      that.hrspDup(that.jsonData(data[that.name+"-"+model.id]), options, syncDfd);
    });
  },
  ...
  findAll: function(options, syncDfd, handleResp) {
    ...
    if (this.records.length == 0) {
      ref.hrspDup(items, options, syncDfd);
      return;
    }
    for(var i=0; i<this.records.length; i++) {
      ...
      chrome.storage.local.get(recordId, function(data) {
        ...
        ref.hrspDup(_.compact(items), options, syncDfd);
      });
    }
  },
  ...
  destroy: function(model, options, syncDfd, handleResp) {
    if (model.isNew())
    var that = this;
    that.hrspDup(false, options, syncDfd);
...
Backbone.LocalStorage.sync = window.Store.sync = Backbone.localSync = function(method, model, options) {
  ...
  console.log(handleResp.name); // "handleresp"
